...
- **The repo path contains a space** (`.../Mariner Digital Agency/Projects/osrs/gunnars-tools`).
...
- Only read/write files inside the `.runelite` directory. Create a subdirectory for your plugin (e.g. `.runelite/gunnars-tools/`) if you need to store data on disk.
...
## Wilderness Slayer specifics for this plugin

Gunnar's Tools reads the player's own inventory/equipment changes to measure
ammunition consumption — it does not touch combat prediction, prayer, or
opponent information in any form, so none of the Boss & Combat or PvP
restrictions above are expected to be relevant to it. If a future milestone
(bank highlighting, or anything else) starts to brush up against one of those
sections, that is the signal to stop and re-read this file before writing the
code, not after.
